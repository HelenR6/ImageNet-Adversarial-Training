...
def resnet_shortcut(l, n_out, stride, activation=tf.identity):
    ...
    if n_in != n_out:   # change dimension when channel is not the same
        return Conv2D('convshortcut', l, n_out, 1, strides=stride, activation=activation, data_format="channels_last")
    else:
        return l
...
def get_bn(zero_init=False):
    if zero_init:
        return lambda x, name=None: BatchNorm('bn', x, gamma_initializer=tf.zeros_initializer(), data_format="channels_last")
    else:
        return lambda x, name=None: BatchNorm('bn', x, data_format="channels_last")
...
def resnet_bottleneck(l, ch_out, stride, group=1, res2_bottleneck=64):
    ...
    shortcut = l
    l = Conv2D('conv1', l, ch_out * ch_factor, 1, strides=1, activation=BNReLU, data_format="channels_last")
    l = Conv2D('conv2', l, ch_out * ch_factor, 3, strides=stride, activation=BNReLU, split=group, data_format="channels_last")
    """
    ImageNet in 1 Hour, Sec 5.1:
    the stride-2 convolutions are on 3×3 layers instead of on 1×1 layers
    """
    l = Conv2D('conv3', l, ch_out * 4, 1, activation=get_bn(zero_init=True), data_format="channels_last")
    """
...
def resnet_backbone(image, num_blocks, group_func, block_func):
    ...
    #         argscope(Conv2D, use_bias=False,
    #                  kernel_initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')):
        l = Conv2D('conv0', image, 64, 7, strides=2, activation=BNReLU, data_format="channels_last")
        l = MaxPooling('pool0', l, pool_size=3, strides=2, padding='SAME', data_format="channels_last")
        ...
        l = GlobalAvgPooling('gap', l, data_format="channels_last")
...
def denoising(name, l, embed=True, softmax=True):
    ...
    with tf.variable_scope(name):
        ...
        f = Conv2D('conv', f, l.shape[1], 1, strides=1, activation=get_bn(zero_init=True), data_format="channels_last")
        l = l + f
    return l
